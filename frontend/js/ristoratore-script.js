...
async function caricaDatiRistorante() {
    try {
        ...
        if (res.ok) {
            ...
            const restNome = document.getElementById('restNome');
            const restTelefono = document.getElementById('restTelefono');
            const restPiva = document.getElementById('restPiva');
            const restIndirizzo = document.getElementById('restIndirizzo');

            if (restNome) restNome.value = data.nomeRistorante || '';
            if (restTelefono) restTelefono.value = data.telefono || '';
            if (restPiva) restPiva.value = data.partitaIva || '';
            if (restIndirizzo) restIndirizzo.value = data.indirizzo || '';
...
const formRistorante = document.getElementById('formRistorante');
if (formRistorante) {
    formRistorante.addEventListener('submit', async (e) => {
        e.preventDefault();
        const data = {
            email: utente.email,
